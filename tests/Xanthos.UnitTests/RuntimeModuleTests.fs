...
module NormalizeDataspecTests =
    ...
    let ``normalizeDataspec should accept valid 8-character spec`` () =
        ...
        | Error err ->
            failwithf
                "RuntimeModuleTests: normalizeDataspec should accept valid 8-character spec: Expected Ok, got %A"
                err
    ...
    let ``normalizeDataspec should trim whitespace`` () =
        match Validation.normalizeDataspec "  RACE  " with
        | Ok result -> Assert.Equal("RACE", result)
        | Error err -> failwithf "RuntimeModuleTests: normalizeDataspec should trim whitespace: Expected Ok, got %A" err
    ...
    let ``normalizeDataspec should convert to uppercase`` () =
        ...
        | Ok result -> Assert.Equal("RACE", result)
        | Error err ->
            failwithf "RuntimeModuleTests: normalizeDataspec should convert to uppercase: Expected Ok, got %A" err
    ...
    let ``normalizeDataspec should accept alphanumeric characters`` () =
        ...
        | Error err ->
            failwithf
                "RuntimeModuleTests: normalizeDataspec should accept alphanumeric characters: Expected Ok, got %A"
                err
    ...
    let ``normalizeDataspec should reject whitespace only`` () =
        ...
        | Error other ->
            failwithf
                "RuntimeModuleTests: normalizeDataspec should reject whitespace only: Expected ValidationError, got %A"
                other
    ...
    let ``normalizeDataspec should reject non-multiple of 4`` () =
        ...
        | Error other ->
            failwithf
                "RuntimeModuleTests: normalizeDataspec should reject non-multiple of 4: Expected ValidationError, got %A"
                other
    ...
    let ``normalizeDataspec should reject 5-character string`` () =
        ...
        | Error other ->
            failwithf
                "RuntimeModuleTests: normalizeDataspec should reject 5-character string: Expected ValidationError, got %A"
                other
    ...
    let ``normalizeDataspec should reject special characters`` () =
        ...
        | Error other ->
            failwithf
                "RuntimeModuleTests: normalizeDataspec should reject special characters: Expected ValidationError, got %A"
                other
    ...
    let ``normalizeDataspec should reject spaces in middle`` () =
        ...
        | Error other ->
            failwithf
                "RuntimeModuleTests: normalizeDataspec should reject spaces in middle: Expected ValidationError, got %A"
                other
    ...
    let ``normalizeDataspec should reject spaces in 4-char block`` () =
        ...
        | Error other ->
            failwithf
                "RuntimeModuleTests: normalizeDataspec should reject spaces in 4-char block: Expected ValidationError, got %A"
                other
...
module ParseOpenOptionTests =
    ...
    let ``parseOpenOption should return 1 for None`` () =
        ...
        | Error err -> failwithf "RuntimeModuleTests: parseOpenOption should return 1 for None: Expected Ok, got %A" err

    [<Fact>]
    let ``parseOpenOption should accept 1`` () =
        match Validation.parseOpenOption (Some "1") with
        | Ok result -> Assert.Equal(1, result)
        | Error err -> failwithf "RuntimeModuleTests: parseOpenOption should accept 1: Expected Ok, got %A" err

    [<Fact>]
    let ``parseOpenOption should accept 2`` () =
        match Validation.parseOpenOption (Some "2") with
        | Ok result -> Assert.Equal(2, result)
        | Error err -> failwithf "RuntimeModuleTests: parseOpenOption should accept 2: Expected Ok, got %A" err

    [<Fact>]
    let ``parseOpenOption should accept 3`` () =
        match Validation.parseOpenOption (Some "3") with
        | Ok result -> Assert.Equal(3, result)
        | Error err -> failwithf "RuntimeModuleTests: parseOpenOption should accept 3: Expected Ok, got %A" err

    [<Fact>]
    let ``parseOpenOption should accept 4`` () =
        match Validation.parseOpenOption (Some "4") with
        | Ok result -> Assert.Equal(4, result)
        | Error err -> failwithf "RuntimeModuleTests: parseOpenOption should accept 4: Expected Ok, got %A" err

    [<Fact>]
    let ``parseOpenOption should trim whitespace`` () =
        match Validation.parseOpenOption (Some "  2  ") with
        | Ok result -> Assert.Equal(2, result)
        | Error err -> failwithf "RuntimeModuleTests: parseOpenOption should trim whitespace: Expected Ok, got %A" err
    ...
    let ``parseOpenOption should reject 0`` () =
        ...
        | Error other ->
            failwithf "RuntimeModuleTests: parseOpenOption should reject 0: Expected ValidationError, got %A" other
    ...
    let ``parseOpenOption should reject 5`` () =
        ...
        | Error(ValidationError msg) -> Assert.Contains("between 1 and 4", msg)
        | Error other ->
            failwithf "RuntimeModuleTests: parseOpenOption should reject 5: Expected ValidationError, got %A" other
    ...
    let ``parseOpenOption should reject negative`` () =
        ...
        | Error other ->
            failwithf
                "RuntimeModuleTests: parseOpenOption should reject negative: Expected ValidationError, got %A"
                other
    ...
    let ``parseOpenOption should reject non-integer`` () =
        ...
        | Error other ->
            failwithf
                "RuntimeModuleTests: parseOpenOption should reject non-integer: Expected ValidationError, got %A"
                other
    ...
    let ``parseOpenOption should reject float`` () =
        ...
        | Error(ValidationError msg) -> Assert.Contains("not a valid integer", msg)
        | Error other ->
            failwithf "RuntimeModuleTests: parseOpenOption should reject float: Expected ValidationError, got %A" other
...
module ParseFromTimeTests =
    ...
    let ``parseFromTime should reject None`` () =
        ...
        | Error other ->
            failwithf "RuntimeModuleTests: parseFromTime should reject None: Expected ValidationError, got %A" other
    ...
    let ``parseFromTime should accept yyyyMMddHHmmss format`` () =
        ...
        | Error err ->
            failwithf "RuntimeModuleTests: parseFromTime should accept yyyyMMddHHmmss format: Expected Ok, got %A" err
    ...
    let ``parseFromTime should accept yyyyMMdd format`` () =
        ...
        | Error err ->
            failwithf "RuntimeModuleTests: parseFromTime should accept yyyyMMdd format: Expected Ok, got %A" err

    [<Fact>]
    let ``parseFromTime should trim whitespace`` () =
        match Validation.parseFromTime (Some "  20240115  ") with
        | Ok result -> Assert.Equal(2024, result.Year)
        | Error err -> failwithf "RuntimeModuleTests: parseFromTime should trim whitespace: Expected Ok, got %A" err
    ...
    let ``parseFromTime should reject invalid format`` () =
        ...
        | Error other ->
            failwithf
                "RuntimeModuleTests: parseFromTime should reject invalid format: Expected ValidationError, got %A"
                other
    ...
    let ``parseFromTime should reject partial date`` () =
        ...
        | Error other ->
            failwithf
                "RuntimeModuleTests: parseFromTime should reject partial date: Expected ValidationError, got %A"
                other
    ...
    let ``parseFromTime should reject text`` () =
        ...
        | Error(ValidationError msg) -> Assert.Contains("Unable to parse", msg)
        | Error other ->
            failwithf "RuntimeModuleTests: parseFromTime should reject text: Expected ValidationError, got %A" other
...
module BuildOpenRequestTests =
    ...
    let ``buildOpenRequest should succeed with valid inputs`` () =
        ...
        | Error err ->
            failwithf "RuntimeModuleTests: buildOpenRequest should succeed with valid inputs: Expected Ok, got %A" err
    ...
    let ``buildOpenRequest should succeed with all parameters`` () =
        ...
        | Error err ->
            failwithf "RuntimeModuleTests: buildOpenRequest should succeed with all parameters: Expected Ok, got %A" err
    ...
    let ``buildOpenRequest should fail on invalid spec`` () =
        ...
        | Error other ->
            failwithf
                "RuntimeModuleTests: buildOpenRequest should fail on invalid spec: Expected ValidationError, got %A"
                other
    ...
    let ``buildOpenRequest should fail on missing fromTime`` () =
        ...
        | Error other ->
            failwithf
                "RuntimeModuleTests: buildOpenRequest should fail on missing fromTime: Expected ValidationError, got %A"
                other
    ...
    let ``buildOpenRequest should fail on invalid option`` () =
        ...
        | Error other ->
            failwithf
                "RuntimeModuleTests: buildOpenRequest should fail on invalid option: Expected ValidationError, got %A"
                other
    ...
    let ``buildOpenRequest should normalize spec to uppercase`` () =
        ...
        | Error err ->
            failwithf "RuntimeModuleTests: buildOpenRequest should normalize spec to uppercase: Expected Ok, got %A" err
...
module ReadRetryPolicyTests =
    ...
    let ``defaultPolicy should return Abort for Payload`` () =
        ...
        | Abort -> ()
        | other ->
            failwithf "RuntimeModuleTests: defaultPolicy should return Abort for Payload: Expected Abort, got %A" other
...
module JvLinkConfigTests =
    ...
    let ``create should succeed with valid SID`` () =
        ...
        | Error err -> failwithf "RuntimeModuleTests: create should succeed with valid SID: Expected Ok, got %A" err

    [<Fact>]
    let ``create should trim SID`` () =
        match JvLinkConfig.create "  test-sid  " None None None with
        | Ok config -> Assert.Equal("test-sid", config.Sid)
        | Error err -> failwithf "RuntimeModuleTests: create should trim SID: Expected Ok, got %A" err
    ...
    let ``create should accept SavePath`` () =
        ...
        | Error err -> failwithf "RuntimeModuleTests: create should accept SavePath: Expected Ok, got %A" err
    ...
    let ``create should accept ServiceKey`` () =
        ...
        | Error err -> failwithf "RuntimeModuleTests: create should accept ServiceKey: Expected Ok, got %A" err
    ...
    let ``create should accept all optional parameters`` () =
        ...
        | Error err ->
            failwithf "RuntimeModuleTests: create should accept all optional parameters: Expected Ok, got %A" err
    ...
    let ``create should fail for empty SID`` () =
        ...
        | Error(ValidationError msg) -> Assert.Contains("non-empty", msg)
        | Error other ->
            failwithf "RuntimeModuleTests: create should fail for empty SID: Expected ValidationError, got %A" other
    ...
    let ``create should fail for whitespace SID`` () =
        ...
        | Error other ->
            failwithf
                "RuntimeModuleTests: create should fail for whitespace SID: Expected ValidationError, got %A"
                other
    ...
    let ``create should fail for null SID`` () =
        ...
        | Error(ValidationError msg) -> Assert.Contains("non-empty", msg)
        | Error other ->
            failwithf "RuntimeModuleTests: create should fail for null SID: Expected ValidationError, got %A" other
