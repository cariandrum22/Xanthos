...
let private invalid api message =
    raise (
        CommandFailure
            { Api = api
              Code = None
...
let private parse spec (data: byte[]) =
...
let private useGets (ctx: ExecutionContext) =
...
let private withConnection (config: Runtime.JvLinkConfig) action =
    let mutable exitCode = 0

    match JvLink.connect (connection ()) with
    | Error e ->
        printfn "COM client creation failed: %s" e.Message
        2
    | Ok session ->
        try
            try
        ...
        with
        | CommandFailure e ->
            printfn "%s failed: code=%A %s" e.Api e.Code e.Message
            exitCode <- 2
        | ex ->
...
let private opened request session =
...
let private readRecords useGets spec maximum output onRecord (cancel: CancellationToken) session =
    let mutable count = 0
...
let private withCancellation action =
...
let private download ctx (args: DownloadArgs) check session =
    let request =
...
let private realtime useGets (args: RealtimeArgs) (token: CancellationToken) session =
...
let private movie (args: MovieOpenArgs) (token: CancellationToken) session =
    try
        match JvLink.movieOpen args.MovieOpenType args.MovieSearchKey session |> require with
        | VideoOpenOutcome.NoData -> printfn "NO_DATA movie type=%s search=%s" args.MovieOpenType args.MovieSearchKey
...
let private watch config (args: WatchArgs) (token: CancellationToken) session =
    let queue = ConcurrentQueue<JvEvent>()
    use subscription = JvLink.subscribe queue.Enqueue session |> require
...
let run ctx command =
    withConnection ctx.Config (fun session ->
        ...
        | CurrentReadSize -> printfn "Current file size: %d bytes" (JvLink.getCurrentReadFileSize session |> require)
                            ...
                            readRecords
                                ...
                                (fun id record ->
                                    ...
                                    if count < args.MaxRecordsPerType then
                                        let filename =
                                            Path.Combine(args.FixturesOutputDir, $"{spec}_{id}_{count + 1:D3}.bin")
