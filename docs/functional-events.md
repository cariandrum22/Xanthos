...
The legacy service receives native events directly into its own configured queue, including events delivered during registration or restart. Its consumer reports queue loss through `EventQueueOverflow`. There is no additional functional delivery queue in that path. Direct `IJvLinkClient.WatchEvent` callbacks run on the native STA and must return promptly; use the service or functional subscriptions for application callbacks.
